...
__device__ void kernel_DoDrugSim_single(double *d_ic50, double *d_cvar, double d_conc, double *d_CONSTANTS,
    ...
    double vm_repol30, vm_repol50, vm_repol90;
    double  t_depol = (d_CONSTANTS[(sample_id * num_of_constants) + BCL] * 0) +
    ...
    for (int temp = 0; temp < num_of_states; temp++) {
        ...
        // printf("%lf,%lf\n", cache[1], cache[2+(num_of_states+2)]); -> this gives you the V for first and second
        // sample
    }
    ...
    while (tcurr[sample_id] < tmax) {
        ...
        if (d_STATES[(sample_id * num_of_states) + V] > inet_vm_threshold) {
            inet +=
                (d_ALGEBRAIC[(sample_id * num_of_algebraic) + INaL] +
                 d_ALGEBRAIC[(sample_id * num_of_algebraic) + ICaL] +
                 d_ALGEBRAIC[(sample_id * num_of_algebraic) + Ito] + d_ALGEBRAIC[(sample_id * num_of_algebraic) + IKr] +
                 d_ALGEBRAIC[(sample_id * num_of_algebraic) + IKs] +
                 d_ALGEBRAIC[(sample_id * num_of_algebraic) + IK1]) *
                dt[sample_id];
        ...
        if (floor((tcurr[sample_id] + dt_set) / bcl) == floor(tcurr[sample_id] / bcl)) {
            ...
            // temp_result[sample_id].cad90 = cad90_curr - cad90_prev; // cad50 and 90 cur not calculcated yet! use
            // outer loop instead
            ...
            // resetting inet and AUC values and increase the pace count

            // NOTE: pacecount is disabled to make sure calculation is only one time
            // pace_count++;
            input_counter = 0;   // at first, we reset the input counter since we re gonna only take one, but I remember
                                 // we don't have this kind of thing previously, so do we need this still?
            ...
            t_depol = (d_CONSTANTS[(sample_id * num_of_constants) + BCL] * pace_count) +
                      d_CONSTANTS[(sample_id * num_of_constants) + stim_start];
            ...
            // printf("core: %d pace count: %d t: %lf, steepest: %d, dvmdt_repol: %lf, t_peak:
        ...
        if (tcurr[sample_id] > ((d_CONSTANTS[(sample_id * num_of_constants) + BCL] * pace_count) +
                                (d_CONSTANTS[(sample_id * num_of_constants) + stim_start] + 2.)) &&
            tcurr[sample_id] < ((d_CONSTANTS[(sample_id * num_of_constants) + BCL] * pace_count) +
                                (d_CONSTANTS[(sample_id * num_of_constants) + stim_start] + 10.)) &&
        ...
        // these operations will be executed if it's eligible AP and executed at the beginning of repolarization
        else if (tcurr[sample_id] > ((d_CONSTANTS[(sample_id * num_of_constants) + BCL] * pace_count) +
                                     (d_CONSTANTS[(sample_id * num_of_constants) + stim_start] + 10)) &&
                 is_eligible_AP) {
            // printf("check 3\n");
            ...
            if (vm_repol50 > d_STATES[(sample_id * num_of_states) + V] &&
                d_STATES[(sample_id * num_of_states) + V] > vm_repol50 - 2)
                temp_result[sample_id].apd50 = tcurr[sample_id] - t_depol;
            if (vm_repol90 > d_STATES[(sample_id * num_of_states) + V] &&
                d_STATES[(sample_id * num_of_states) + V] > vm_repol90 - 2)
            ...
            if (temp_result[sample_id].ca_peak < d_STATES[(sample_id * num_of_states) + cai]) {
                ...
                ca_amp50 = temp_result[sample_id].ca_peak -
                           (0.5 * (temp_result[sample_id].ca_peak - temp_result[sample_id].ca_valley));
                ca_amp90 = temp_result[sample_id].ca_peak -
                           (0.9 * (temp_result[sample_id].ca_peak - temp_result[sample_id].ca_valley));
                t_ca_peak = tcurr[sample_id];
                // printf("ca_amp50 = %lf - (0.5 * (%lf - %lf)) = %lf\n",temp_result[sample_id].ca_peak,
                // temp_result[sample_id].ca_peak,temp_result[sample_id].ca_valley, ca_amp50); printf("ca_amp90 = %lf -
                // (0.9 * (%lf - %lf)) = %lf\n",temp_result[sample_id].ca_peak,
                // temp_result[sample_id].ca_peak,temp_result[sample_id].ca_valley, ca_amp90);
            }
        }
        ...
        // // inet_ap = (d_ALGEBRAIC[(sample_id * num_of_algebraic) +INaL]+d_ALGEBRAIC[(sample_id * num_of_algebraic)
        // +ICaL]+d_ALGEBRAIC[(sample_id * num_of_algebraic) +Ito]+d_ALGEBRAIC[(sample_id * num_of_algebraic)
        // +IKr]+d_ALGEBRAIC[(sample_id * num_of_algebraic) +IKs]+d_ALGEBRAIC[(sample_id * num_of_algebraic) +IK1]);
        // // inet4_ap = (d_ALGEBRAIC[(sample_id * num_of_algebraic) +INaL]+d_ALGEBRAIC[(sample_id * num_of_algebraic)
        // +ICaL]+d_ALGEBRAIC[(sample_id * num_of_algebraic) +IKr]+d_ALGEBRAIC[(sample_id * num_of_algebraic) +INa]);
        ...
        // inet_cl = (d_ALGEBRAIC[(sample_id * num_of_algebraic) +INaL]+d_ALGEBRAIC[(sample_id * num_of_algebraic)
        // +ICaL]+d_ALGEBRAIC[(sample_id * num_of_algebraic) +Ito]+d_ALGEBRAIC[(sample_id * num_of_algebraic)
        // +IKr]+d_ALGEBRAIC[(sample_id * num_of_algebraic) +IKs]+d_ALGEBRAIC[(sample_id * num_of_algebraic) +IK1]);
        // inet4_cl = (d_ALGEBRAIC[(sample_id * num_of_algebraic) +INaL]+d_ALGEBRAIC[(sample_id * num_of_algebraic)
        // +ICaL]+d_ALGEBRAIC[(sample_id * num_of_algebraic) +IKr]+d_ALGEBRAIC[(sample_id * num_of_algebraic) +INa]);
    ...
    for (int ca_looper = 0; ca_looper < p_param->sampling_limit; ca_looper++) {
        ...
        // after the peak calcium
            ...
            if (temp_result[sample_id].cai_data[ca_looper] > ca_amp50)
                cad50_curr = temp_result[sample_id].cai_time[ca_looper];
            if (temp_result[sample_id].cai_data[ca_looper] > ca_amp90)
                cad90_curr = temp_result[sample_id].cai_time[ca_looper];
        }
    }
